...
# Title
st.title(':black[Youtube Data Harvesting Streamlit Application]')
...
with col1:
    st.header(':green[1. Data collection zone]')
    st.write ('(This zone is to collect data by using channel id and store it in MongoDB database.)')
    channel_id = st.text_input('**Enter channel_id**')
    st.write('''Click below button to get the data and store it in MongoDB.''')
 ...
 # -----------------------------------    /   MongoDB connection and store the collected data   /    ---------------------------------- #
        ...
        # create a client instance of MongoDB cloud(Below URL is specific for every user)
        client = pymongo.MongoClient('mongodb+srv://<username>:<password>@cluster0.h2r55k6.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0')
...
with col2:
    st.header(':green[2. Data Migration zone]')
    st.write ('''(This zone is to migrate the stored data in MongoDB database to MYSQL for a specific channel.)''')
    ...
    st.write('''Click below button to migrate the data to MYSQL from MongoDB.''')
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        # Connect to the MySQL server cloud(FreeSQLDatabase)
        ...
        mycursor.execute("CREATE DATABASE IF NOT EXISTS sql6690945")
        ...
        engine = create_engine('mysql+mysqlconnector://<username>:<password>.freesqldatabase.com/sql6690945', echo=False)
...
st.header(':green[3. Channel Data Analysis zone]')
st.write ('''(This zone is to analyze the data and display the result in table format for predefined set of questions.)''')
...
connect_for_question = pymysql.connect(host='sql6.freesqldatabase.com', user='sql6690945', password='zkIw5vIr1r', db='sql6690945')
